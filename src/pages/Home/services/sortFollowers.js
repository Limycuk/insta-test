...
const sortFollowers = (followers, sortValue) => {
  switch (sortValue) {
    case 'likes-za': {
      return followers.sort(compare.bind({ name: 'likes.count', isDescSort: 1 }))
    }
    case 'likes-az': {
      return followers.sort(compare.bind({ name: 'likes.count', isDescSort: -1 }))
    }
    ...
    case 'followers-za': {
      return followers.sort(compare.bind({ name: 'followers', isDescSort: 1 }))
    }
    case 'followers-az': {
      return followers.sort(compare.bind({ name: 'followers', isDescSort: -1 }))
    }
    case 'followings-za': {
      return followers.sort(compare.bind({ name: 'followings', isDescSort: 1 }))
    }
    case 'followings-az': {
      return followers.sort(compare.bind({ name: 'followings', isDescSort: -1 }))
    }
